 ...
 *
 * @link       https://github.com/AgriLife/agrilife-degree-programs/blob/master/src/class-taxonomy.php
 * @since      0.1.0
	...
	public function save_taxonomy_custom_meta( $term_id, $tt_id ) {
		...
		foreach ( $this->meta_boxes as $key => $meta ) {
			...
			if (
				...
				|| ! wp_verify_nonce( sanitize_key( $_POST[ $key . '_nonce' ] ), basename( __FILE__ ) )
			) {
				continue;
			}
